    ...
    <div className="p-6 bg-slate-900 border border-slate-800 rounded-2xl h-80 shadow-xl flex flex-col justify-between">
      <div>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-slate-300">Top 5 Diretórios Hotspots (Concentração de Risco)</h3>
        </div>

        {hotspots.length === 0 ? (
          <div className="flex items-center justify-center h-48 text-xs text-slate-500">
            Nenhum diretório vulnerável mapeado.
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-2 text-[11px] font-sans">
                    {item.criticalCount > 0 && (
                      <span className="text-rose-400 font-semibold flex items-center gap-0.5">
                        <AlertCircle className="w-3 h-3" /> {item.criticalCount} Críticas
                      </span>
                    )}
                    <span className="text-slate-400">{item.findingCount} achados ({item.percentage}%)</span>
                  </div>
                </div>
      ...
      <p className="text-[11px] text-slate-500 mt-2">
        Identifica módulos do código que acumulam a maior parcela do débito técnico de segurança.
      </p>
    </div>
  );
};
