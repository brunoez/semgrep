...
export const FileDropzone: React.FC = () => {
  ...
  const readFile = (file: File) => {
    if (!file.name.endsWith('.json')) {
      alert('Por favor, envie um arquivo com extensão .json');
      return;
    }
    ...
    <div className="w-full">
      ...
      >
        ...
        <h3 className="text-base font-bold text-white mb-1">
          Solte seu arquivo <code className="text-indigo-400 font-mono">semgrep.json</code>
        </h3>
        <p className="text-xs text-slate-400 max-w-xs mb-6">
          Arraste o relatório exportado do terminal para começar a visualização instantânea.
        </p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 z-20 pointer-events-auto">
          ...
          >
            ...
            {pasteOpen ? 'Fechar Paste' : 'Colar JSON'}
          </button>
        ...
        {pasteOpen && (
          <div className="w-full mt-4 z-20 pointer-events-auto text-left space-y-2">
            <textarea
              rows={4}
              placeholder="Cole aqui o conteúdo do seu resultado semgrep.json..."
              value={pasteContent}
            ...
            >
              {isLoading ? 'Processando JSON...' : 'Analisar Conteúdo Colado'}
            </button>
          </div>
        )}
      </div>
